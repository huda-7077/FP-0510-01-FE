...
import useCreateSkillAssessmentQuestion from "@/hooks/api/skill-assessment-question/useCreateSkillAssessmentQuestion";
import useUpdateSkillAssessmentQuestion from "@/hooks/api/skill-assessment-question/useUpdateSkillAssessmentQuestion";
...
export function QuestionForm({
  skillAssessmentId,
  skillAssessmentQuestionId,
  editQuestion,
  onCancelEdit,
  isProcessing,
}: QuestionFormProps) {
  const {
    mutateAsync: createSkillAssessmentQuestion,
    isPending: createPending,
  } = useCreateSkillAssessmentQuestion();
  const {
    mutateAsync: updateSkillAssessmentQuestion,
    isPending: updatePending,
  } = useUpdateSkillAssessmentQuestion();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (editQuestion) {
        if (!skillAssessmentQuestionId) {
          toast.error("Skill Assessment Question ID is required");
          return;
        }
        await updateSkillAssessmentQuestion({
          skillAssessmentQuestionId,
          ...payload,
        });
      } else {
        if (!skillAssessmentId) {
          toast.error("Skill Assessment ID is required");
          return;
        }
        await createSkillAssessmentQuestion({
          skillAssessmentId,
          ...payload,
        });
      }
