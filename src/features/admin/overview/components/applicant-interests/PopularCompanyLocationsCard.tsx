...
interface PopularCompanyLocationsCardProps {
  data: { province: string; applicants: number }[];
  onTimeRangeChange: (timeRange: string) => void;
}

const PopularCompanyLocationsCard: FC<PopularCompanyLocationsCardProps> = ({
  data,
  onTimeRangeChange,
}) => {
  return (
    <Card className="group flex h-full flex-col rounded-2xl border-2 bg-white shadow-none transition-all duration-300 ease-in-out hover:border-blue-600">
      ...
      <CardFooter className="mt-auto border-t border-gray-100 pt-4">
        <p className="text-xs italic text-gray-500">
          *Data shows top 10 company locations (province) with the highest
...
export default PopularCompanyLocationsCard;
